...
class Button : public GUnit
{
	...
	GWEN_CONTROL_INLINE( Button, GUnit )
	{
			...
			pButtonA->SetText( GWEN_T("Event Tester") );
			...
			pButtonF->SetText( GWEN_T("Disabled :D") );
	...
	{
		UnitPrint( GWEN_T("Button Toggled (using 'OnToggle' event)") );
	}

	void OnToggleOn( Controls::Base* pControl )
	{
		UnitPrint( GWEN_T("Button Toggled ON (using 'OnToggleOn' event)") );
	}

	void OnToggleOff( Controls::Base* pControl )
	{
		UnitPrint( GWEN_T("Button Toggled Off (using 'OnToggleOff' event)") );
	}
...
DEFINE_UNIT_TEST( Button, GWEN_T("Button") );
